...
class DroneState:
    """Store the current simulation state of one drone."""

    def __init__(self, drone_id: int, path: list[str]) -> None:
        """Initialize a drone with its assigned path."""
        ...
        # if in_transit, how many turns until arrival
...
def simulate_single_drone(
    ...
    """Simulate one drone following its assigned path."""
    # format is: D<ID>-<zone>
    turns: list[list[str]] = []
...
def simulate_all_drones(world: Map) -> list[list[str]]:
    """Simulate all drones while respecting capacities and waiting."""
    # if world.end is None, stop the program. Otherwise continue
    # assuming it isn't None.
    ...
    # way to look up a connection's max link capacity
    # just from its name string:
    connection_capacity: dict[str, int] = {}
    for conn in world.connections:
        connection_capacity[f"{conn.zone_a}-{conn.zone_b}"] = (
            conn.max_link_capacity
        )
        connection_capacity[f"{conn.zone_b}-{conn.zone_a}"] = (
            conn.max_link_capacity
        )
    ...
    zone_occupancy: dict[str, int] = {
        world.start.name: world.nb_drones
    }
    # same but for connections (how many drones are currently
    # mid-transit thru each one)
    ...
    while not all_delivered(drones):
        turn_count += 1
        if turn_count > 200:
            stuck: list[DroneState] = []
            for d in drones:
                if not d.delivered:
                    stuck.append(d)

            print(
                f"STOPPED at turn {turn_count}, "
                f"{len(stuck)} drones stuck"
            )
        ...
        # movement strings thatll actually get printed for this turn,
        # it appends to the turns at the end
        turn_moves: list[str] = []
        ...
        for drone in drones:
            ...
            if drone.in_transit:
                ...
                if drone.transit_turns_left == 0:
                    ...
                    connection_name = (
                        f"{current_zone_name}-{arrived_zone}"
                    )
                    connection_occupancy[connection_name] -= 1
                    zone_reserved[arrived_zone] -= 1
                    zone_occupancy[arrived_zone] = (
                        zone_occupancy.get(arrived_zone, 0) + 1
                    )
                    turn_moves.append(
                    ...
                    if arrived_zone == world.end.name:
                        ...
                        zone_occupancy[arrived_zone] -= 1
                continue

            wanting_to_move.append(drone)

        # accounting for same-turn departures, before checking zone capacity:
        # (this now runs ONCE, after wanting_to_move is fully built,
        # not nested per-drone)
        temp_zone_occupancy: dict[str, int] = dict(zone_occupancy)
        for drone in wanting_to_move:
            current_zone_name = drone.path[drone.path_index]
            temp_zone_occupancy[current_zone_name] = (
                temp_zone_occupancy.get(current_zone_name, 0) - 1
            )
        ...
        for drone in wanting_to_move:
            ...
            if cost == 1:
                ...
                connection_name = (
                    f"{current_zone_name}-{next_zone_name}"
                )
        ...
        for connection_name in connection_requests:
            ...
            current_usage = connection_occupancy.get(
                connection_name, 0
            )
            ...
            destination_zone_name = (
                requesters[0].path[
                    requesters[0].path_index + 1
                ]
            )
            destination_zone = world.zones[destination_zone_name]
            zone_free_slots = (
                destination_zone.max_drones
                - temp_zone_occupancy.get(
            ...
            for drone in requesters:
                ...
                granted.append(drone)
                free_slots -= 1
                zone_reserved[destination_zone_name] = (
                    zone_reserved.get(
                        destination_zone_name, 0
                    ) + 1
                )
        ...
        for drone in wanting_to_move:
            ...
            if cost == 1:
                drone.path_index += 1
                zone_occupancy[next_zone_name] = (
                    zone_occupancy.get(next_zone_name, 0) + 1
                )
                turn_moves.append(
                    f"D{drone.drone_id}-{next_zone_name}"
                )
                ...
                connection_occupancy[connection_name] = (
                    connection_occupancy.get(connection_name, 0) + 1
                )
                turn_moves.append(
