...
def draw_rainbow_zone(
    ...
    diagonal = generate_rainbow_diagonal()
...
def compute_marker_size(world: Map) -> float:
...
def draw_static_layout(axes, world: Map, marker_size: float, hex_radius: float) -> list:
    ...
    for zone_name in world.zones:
        ...
        if color == "rainbow":
            ...
            axes.scatter(zone.x, zone.y, marker=marker, s=marker_size, c=color,
                             edgecolors="black", zorder=3)

        xs = [zone.x for zone in world.zones.values()]
        ys = [zone.y for zone in world.zones.values()]
        map_size = max(max(xs) - min(xs), max(ys) - min(ys))
        zone_label_offset = min(0.6, max(0.1, map_size * 0.015))
        ...
        xs = [zone.x for zone in world.zones.values()]
    ys = [zone.y for zone in world.zones.values()]
    adjust_text(list(zone_labels.values()), x=xs, y=ys, ax=axes,
...
def animate_simulation(world: Map, history: list[dict[int, tuple[float, float]]],
    ...
    fig_width = max(10.0, map_width * 1.1)
    fig_height = max(10.0, map_height * 1.1)

    fig, ax = plt.subplots(figsize=(fig_width, fig_height))
    marker_size = compute_marker_size(world)
    hex_radius = compute_hexagon_radius(world)
    rainbow_zones, zone_labels = draw_static_layout(
        ax, world, marker_size, hex_radius)
    ...
    def update(frame_index):
        ...
        for drone_id in positions:
            ...
            if drone_id not in drone_artists:
                image_box = OffsetImage(drone_icon, zoom=0.07)
                annotation = AnnotationBbox(
                    image_box, (x, y), frameon=False, zorder=5)
                ax.add_artist(annotation)
                drone_artists[drone_id] = annotation
    ...
    def update_rainbow(frame_index):
        ...
        for hexagon_patch, image_artist, diagonal in rainbow_zones:
            ...
            image_artist.set_data(colormap(shifted))
    ...
    anim = FuncAnimation(fig, update, frames=len(history), interval=interval_ms,
                         repeat=False)
    rainbow_anim = FuncAnimation(fig, update_rainbow, interval=20, blit=False)
    plt.show()
    plt.close('all')
